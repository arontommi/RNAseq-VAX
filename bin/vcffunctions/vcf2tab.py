...
def extract_columns_varLength(df_column, spliter, col_name):
    extracted_column = pd.DataFrame(df_column.str.split('{}'.format(spliter),
        n=len(df_column.head().str.get_dummies(sep='{}'.format(spliter)).columns), expand=True))
    length = len(extracted_column.columns)
    if length > len(col_name):
        names = ['{0}_{1}'.format(col_name[-1], i) for i in list(range(length-2))]
        names.insert(0, col_name[1])
        names.insert(0, col_name[0])
    else:
        names = col_name
    extracted_column.columns = names
    extracted_column.reset_index(drop=True, inplace=True)
    extracted_column = extracted_column.apply(pd.to_numeric)
    return extracted_column
