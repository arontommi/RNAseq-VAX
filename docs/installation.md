# RNA-seq_AVC Installation

To start using the RNA-seq_AVC pipeline, there are three steps described below:
...
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `rna-seq_AVC` is specified as the pipeline name.
...
```bash
wget https://github.com/rna-seq_AVC/archive/master.zip
unzip master.zip -d /my-pipelines/
cd /my_data/
nextflow run /my-pipelines/rna-seq_AVC-master
```
